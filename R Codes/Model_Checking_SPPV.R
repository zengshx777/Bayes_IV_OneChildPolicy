...
#Main Function to Calculate SPPV
sppv_comparison<-function(j,rep.time,Checking_Sample,Original_Data)
{
  ...
  #Response
  yobs<-Original_Data[,response_index[res]];res.level=length(unique(yobs))
...
Checking_Sample<-get(paste(response_index[res],k,"JAGSObject",sep="_"))$BUGSoutput$sims.matrix;
...
SPPV_temp <- c(
  ...
  c(
    #Signal
    wilcox.test(sample(p_result[[k]][, 1],rep.time), runif(rep.time, 0, 1)),
    #Noise
    wilcox.test(sample(p_result[[k]][, 2],rep.time), runif(rep.time, 0, 1)),
    #SNR
    wilcox.test(sample(p_result[[k]][, 3],rep.time), runif(rep.time, 0, 1))
  ),
  #CP
  c(
    #Signal
    wilcox.test(sample(p_result[[k]][, 4],rep.time), runif(rep.time, 0, 1)),
    #Noise
    wilcox.test(sample(p_result[[k]][, 5],rep.time), runif(rep.time, 0, 1)),
    #SNR
    wilcox.test(sample(p_result[[k]][, 6],rep.time), runif(rep.time, 0, 1))
  ),
  #AT
  c(
    #Signal
    wilcox.test(sample(p_result[[k]][, 7],rep.time), runif(rep.time, 0, 1)),
    #Noise
    wilcox.test(sample(p_result[[k]][, 8],rep.time), runif(rep.time, 0, 1)),
    #SNR
    wilcox.test(sample(p_result[[k]][, 9],rep.time), runif(rep.time, 0, 1))
  )
)

print(paste("==",res,"==finish",k))
}
SPPV_Result<-cbind(SPPV_Result,SPPV_temp)
}
stopImplicitCluster()
